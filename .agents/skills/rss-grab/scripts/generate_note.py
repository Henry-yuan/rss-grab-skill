...
def call_llm_for_note_generation(
    ...
    if is_fallback:
        system_prompt = (
            ...
            "如果没纠错也写'本笔记无 ASR 错字'。\n"
            "5. **引用零隐性修正**：'关键引用'必须与 transcript 原文逐字核对；"
            "凡引用时修正了原文转写错字的（原文写'X'、引用作'Y'），"
            "必须同步列入错字纠正清单，禁止未申报的隐性修正。\n"
        )
        ...
        system_prompt = (
            ...
            "9. **引用零隐性修正**：'关键引用'若修正了摘要 quotes 里的转写错字，"
            "必须同步列入错字纠正清单，禁止未申报的隐性修正。\n\n"
        ...
        system_prompt = (
            ...
            "列出识别并纠正的错字（格式：原 transcript 写'X'，实为'Y'，已修正）。"
            "如果没纠错也写'本笔记无 ASR 错字'。\n"
            "6. **引用零隐性修正**：'关键引用'必须与 transcript 原文逐字核对；"
            "凡引用时修正了原文转写错字的（原文写'X'、引用作'Y'），"
            "必须同步列入错字纠正清单，禁止未申报的隐性修正。\n\n"
            "输出 Markdown，直接是笔记内容，不要加额外说明。"
        )
